...
class PlayAdapter(SourceAdapter):
    ...
    def segments(self, path: Path, skip_lines: int = 0) -> list[Segment]:
        ...
        for line_no, line in enumerate(raw_text.splitlines(), start=1):
            ...
            in_excluded_range = line_no <= skip_lines
...
def load_segments(
    book_path: Path = config.BOOK_EN_PATH, skip_lines: int | None = None
) -> list[Segment]:
    ...
    `skip_lines` : nombre de lignes de tête (1-indexées, lignes vides
    comprises) à traiter comme hors-œuvre. Par défaut (None), reprend
    `config.FRONT_MATTER_SKIP_LINES` (0 sauf override explicite)."""

    if skip_lines is None:
        skip_lines = config.FRONT_MATTER_SKIP_LINES

    adapter = PlayAdapter()
    segments = adapter.segments(book_path, skip_lines=skip_lines)
