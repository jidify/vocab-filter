...
def _completion_response(payload: dict):
    ...
    de response_model Pydantic (JSON libre)."""
    import json as _json
    message = type("Message", (), {"content": _json.dumps(payload)})()
...
class OccurrenceFirstDecisionTests(unittest.TestCase):
    ...
    def test_same_candidate_type_can_receive_distinct_occurrence_verdicts(self):
        ...
        with mock.patch.object(mwe_judge.llm_client.litellm, "batch_completion",
                               side_effect=[[_completion_response(r)] for r in replies]):
...
class ContextualMweDefinitionTests(unittest.TestCase):
    ...
    def test_exact_competing_sense_is_selected_after_all_are_presented(self):
        ...
        with mock.patch.object(mwe_judge, "definition_candidates", return_value=candidates), \
             mock.patch.object(mwe_judge.llm_client.litellm, "batch_completion",
        ...
        _sent_system, sent_user = call.call_args.kwargs["messages"][0]
        prompt = sent_user["content"]
